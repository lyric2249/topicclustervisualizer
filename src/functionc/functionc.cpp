...
)
{
    ...
    //위에서 아르마 데이터 입력하고 패러미터 넣으니까 정상적으로 리턴되는거 확인
    //이제 넘파이를 넣고 이걸 아르마로 변환 후 연산 후 넘파이로 변환해서 리턴
    //해당과정에서 카르마 필요
    //각 성분들을 리스트의 요소로 집어넣는다
    ...
    //리스트 요소로 하려면 어레이여야 해서 억셉트감마 값으로 어레이 작성
    ...
    //cout 정상작동중
    //디버깅에서 왠지는 모르겠는데 arma 매트릭스가 일정 크기 이상이면 출력이 안됨
