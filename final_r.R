...
#dir = "/Users/yeseul/Dropbox/Kelly/??€?•™?›/ì§„ìµ?›ˆêµìˆ˜?‹˜/gViz/90_result"
#dir = "/Users/jeon-yeseul/Dropbox/Kelly/??€?•™?›/ì§„ìµ?›ˆêµìˆ˜?‹˜/gViz/75_result"
...
t(output$accept_beta) == t(output_new$accept_beta)

t(output_new$accept_beta)
t(output_new$accept_theta) == t(output$accept_theta)
t(output_new$accept_w) == t(output$accept_w)
t(output_new$accept_z) == t(output$accept_z)
output_new$accept_gamma == output$accept_gamma
...
#write.table(output_new, file="output_new.txt")
...
abd=
c(
3, 1, 1, 0, 0, 0, 1, 2, 1, 2, 1, 0, 2, 2, 2, 2, 3, 3, 3, 0, 3, 3,
...
#table(word_cluster$cluster)
#table(abd + 1)
#word_cluster$cluster ==  abd + 1
...
"""
eig = eigen(matrix(c(1,2,4,7),2,2))

res = sort(abs(eig$values), index.return = TRUE)
eig$vectors[,res$ix[1:1]]
...
data.frame(c(1,2,3,4,5), c(5,6,7,8,9))
