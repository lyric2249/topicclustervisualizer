...
data = data[:, -0]  # 0번째 column은 data_word. 뽑아내고 나머지 데이타 보존.

data = data.fillna(99)  # na인 cell은 값을 99로 설정.

# data_m = as.matrix(data)

data_m = data  # 데이터 타입 매트릭스로 바뀐 데이터. 파이썬에선 pd.dataframe 사용

logit_x = log(data_m / (1 - data_m))  # 매트릭스 데이터를 logit化
...
for aa in range(3, (ncol(data_set2))):
    ...
    def affinityMatrix(diff, K=20, sigma=0.5):
        ...
        def finiteMean(x):
            return mean(x[isfinite(x)])
            # return(mean(x[is.finite(x)]))

        means = np.apply_along_axis(finiteMean, 1, sortedColumns[:, 1:K + 1]) + np.finfo("double").eps
        # means = apply(sortedColumns[, 1:K + 1], 1, finiteMean) + .Machine$double.eps
        # def avg(x, y):
        #    return (x + y)/2
        Sig = np.mean.outer(means, means) / 3 * 2 + diff / 3 +.Machine$double.eps
