...
//컬럼길이에 맞춰서 슬라이스, 각각에 맞는 형변환하고
//넘파이로 리턴
//전부 큐브로 바꿔서 field로 뭉치는건 샘플만들어서
//다음 함수로 넘기는 과정에서는 한번 뭉쳐서 보내야
//하니까 필요하지 않을까
...
PYBIND11_MODULE(step_first, m) 
{
    // m.doc() = "first_step"; // optional module docstring
...
py::array_t<float> return_results(arma::field<cube> input, int col)
{
    input = qwerty;

    if (col == 0 || 1)
    {
        mat value = carma::mat_to_arr(input(0, col).slice(0));
        return value;
    }
    else if (1 < col < 4)
    {
        cube value = input(0, col);
        return value;
    }
    else if (col == 12)
    {
        double value = input(0, col)(0, 0, 0);
        return value;
    }
