...
// [[Rcpp::export]]
Rcpp::List onepl_lsrm_cont_missing(arma::mat data, 
                                       ...
                                       const double pr_mean_beta, const double pr_sd_beta, const double pr_a_th_sigma, const double pr_b_th_sigma, const double pr_mean_theta,
                                       const double pr_a_sigma, const double pr_b_sigma, const double pr_mean_gamma, const double pr_sd_gamma, const double missing){
  ...
  for(int iter = 0; iter < niter; iter++){
    ...
    for(j = 0; j < nsample; j++) post_b_th_sigma += std::pow((oldtheta(j) - pr_mean_theta), 2.0);
    ...
    for(j = 0; j < nsample; j++){
      for(i = 0; i < nitem; i++) post_b_sigma += std::pow((data(j,i) - oldbeta(i) - oldtheta(j) + oldgamma * dist(j,i)), 2.0) /2;
    }
